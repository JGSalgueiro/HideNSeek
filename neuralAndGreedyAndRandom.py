...
from aasma.neuralNetworks import load_network, save_network
...
class MyTestCase(unittest.TestCase):
    ...
    def test_NeuralCentralizedVectorSeekerVsRandomPrey_CloneReproduction(self):
        with ThreadPoolExecutor(os.cpu_count()) as thread_pool:
            n_agents = 5
            ...
            # 2 - Setup agent
            seeker_families = [
                [NeuralCentralizedVectorAgent(i, environment.n_agents, environment.n_preys,
                                              False, environment, family_id)
                 for i in
                 range(n_agents)] for family_id in range(n_families)]
            preys = [Agent(i, environment.n_agents, environment.n_preys, True, environment) for i in
                     range(n_agents)]
            ...
            while current_generation <= n_max_generations:
                ...
                # Killing off the half worst and replacing with slightly mutated clones from the half best
                for i in range(family_threshold):
                    family_to_reproduce = seeker_families[sorted_families[i][0]][0]
                    family_to_die = seeker_families[sorted_families[i + family_threshold][0]][0]

                    newNetwork = clone(family_to_reproduce.get_my_network())
                    newNetwork.mutate(mutate_chance)

                    family_to_die.set_my_network(newNetwork)
    ...
    def test_NeuralCentralizedVectorSeekerVsGreedyPrey_CloneReproduction(self):
        with ThreadPoolExecutor(os.cpu_count()) as thread_pool:
            n_agents = 5
            n_max_generations = 500
            current_generation = 0
            ...
            seeker_families = [
                [NeuralCentralizedVectorAgent(i, environment.n_agents, environment.n_preys,
                 ...
                 for i in
                 range(n_agents)] for family_id in range(n_families)]
            preys = [GreedyAgent(i, environment.n_agents, environment.n_preys, True, environment) for i in
                     range(n_agents)]
            ...
            while current_generation <= n_max_generations:
                ...
                def run_family(family_i):
                    ...
                    def give_save_name(epi) -> str:
                        return "neuralCentralizedVsGreedy/Family_" + str(family_i) + "_Generation_" + str(current_generation)
