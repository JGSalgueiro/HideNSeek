...
class SocialConventionAgent(agent.Agent):
    def __init__(self, agentId: int, nSeekers: int, nPreys: int, is_prey: bool, environment, wantsToReceiveInformation=True):
    ...
    def action(self) -> int:
        if len(self.visible_enemy_positions) == 0:
            if self.is_prey():
                ...
                if self.step == 0:
                    self.planned_action = np.random.randint(4)
                    self.step = 7
                else:
